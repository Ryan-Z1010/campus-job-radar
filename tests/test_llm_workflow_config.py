...
class LlmWorkflowConfigTests(unittest.TestCase):
    def test_scheduled_workflow_defaults_to_fifty_llm_jobs(self):
        ...
        self.assertIn('USERS_CONFIG_JSON: ${{ secrets.USERS_CONFIG_JSON }}', workflow)
        self.assertIn('llm-multi-analyze', workflow)
        self.assertIn('--users "$users_path"', workflow)
        self.assertIn('data/users/', workflow)
