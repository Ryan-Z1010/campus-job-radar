...
本地多人模式把公共来源和私有用户数据分成两层：

```mermaid
flowchart LR
    S["公共 configs/sources.json"] --> C["CollectionAgent：一次采集"]
    C --> A["用户 A：画像、资格、评分、独立数据库、邮件"]
    C --> B["用户 B：画像、资格、评分、独立数据库、邮件"]
    C --> N["用户 N：画像、资格、评分、独立数据库、邮件"]
```

`multi-monitor` 使用 `configs/users.local.json` 读取用户 ID、画像路径和收件地址，共享采集快照后再为每个用户克隆岗位对象，避免不同画像互相覆盖分数。每个用户使用独立 SQLite 文件和报告目录，通知地址通过 `NotificationAgent` 显式传入；SMTP 发件凭据仍只从运行者环境变量读取。这样同一个岗位可以分别命中不同用户，通知去重不会跨用户串扰。

多人配置属于私有运行数据，不能提交到 GitHub。在线化时应将用户表、画像、匹配结果和通知记录迁移到带权限控制的数据库，并将通知去重键设计为 `(user_id, job_fingerprint)`；公司来源仍保持全局只读，采集和岗位去重保持一次完成。
