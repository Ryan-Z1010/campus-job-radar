...
只有同时显式传入 `--send-email` 和预览目录，程序才会把通过门槛岗位交给 SMTP；没有通过岗位时不会发送。若有新的确定性“待核对/需核对”岗位，或 LLM 的 `needs_review`、分析失败项，同一次运行会额外发送一封独立的“待人工复核”邮件；这封邮件明确说明不代表推荐直接投递，并附官方岗位链接供人工核对。
