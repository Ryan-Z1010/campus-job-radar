...
分析结果还会计算一个只读的可投递门槛：央企/国企必须硬性资格为“符合”、岗位方向被 LLM 确认适配且 Critic 判定 accept；外企/私企在此基础上还需达到默认 70 分。城市不参与加分；不满足条件时只进入人工复核，不触发邮件。画像中的 `accepted_recruitment_windows` 可明确配置可参加的招聘季；当前示例为 `2026秋招` 和 `2027春招`，季节缺失或超出窗口的岗位仍需人工核对。
...
只有同时显式传入 `--send-email` 和预览目录，程序才会把通过门槛岗位交给 SMTP；没有通过岗位时不会发送。若有新的 `needs_review` 或分析失败项，同一次运行会额外发送一封独立的“待人工复核”邮件；这封邮件明确说明不代表推荐直接投递。
