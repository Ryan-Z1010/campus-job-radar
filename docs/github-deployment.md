...
`LLM gated job monitor` 工作流负责在悉尼时间每天 17:00 定时执行 LLM 门槛分析（通过两个 UTC 触发点加时区守卫适配夏令时）：默认最多分析 50 个岗位，并优先处理确定性“待核对/需核对”队列；以 4 个并行采集 worker 获取来源，生成报告和通知预览。只有定时运行或手动显式勾选 `send_email`，且 SMTP Secrets 完整时，才会发送通过门槛的岗位和 LLM 复核后仍不确定的人工复核摘要。`LLM_PROFILE_JSON` 存在时只在 Runner 临时目录使用，不会上传到 Artifact。
